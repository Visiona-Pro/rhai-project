...
import { CHECKOUT_URL, MEMBERS_URL, PRICE_INSTALLMENTS, PRICE_INSTALLMENT_VALUE, PRICE_CASH, PRICE_CURRENCY } from "../config";
...
export default function OfferSection() {
  ...
  return (
    <section aria-label="Oferta exclusiva — Café sem Migalhas" className="relative px-4 pt-1 pb-0 max-w-4xl mx-auto z-10 w-full">
      ...
      >
        {/* Background elegant subtle radial gold pattern overlay inside the card */}
        ...
        {/* Single Image Column/Container - Responsive alignment for Mobile (Left column in grid row 1) and Desktop (Left Column of overall layout) */}
        ...
        {/* Right Column / Content container - Uses contents display on mobile to allow direct positioning of its children inside the card's main grid */}
        <div className="contents sm:flex sm:flex-col sm:flex-1 sm:p-3 md:p-4 sm:justify-between sm:min-w-0">
          
          {/* Upper Text Column - Placed in the right cell of mobile grid row 1 */}
          <div className="col-start-2 row-start-1 mb-2 sm:mb-1.5 flex flex-col justify-center min-w-0">
            ...
            <h2 className="font-serif text-sm xs:text-base sm:text-2xl md:text-[23px] font-bold tracking-tight mb-0.5">
              <span className="gold-brushed-text">
                CAFÉ SEM MIGALHAS
              </span>
            </h2>
          ...
          {/* Benefits Grid Component - Full width on row 2 of mobile grid */}
          <div className="col-start-1 col-end-3 row-start-2 w-full border-t border-[#c4a34f]/10 pt-1.5 mb-1.5">
            ...
            {/* Urgency warning banner placed below the 4 points */}
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.2, duration: 0.6 }}
              className="urgency-bar rounded-xl flex items-center justify-center gap-1.5 text-center mt-2 relative z-10"
              id="page-urgency-banner"
            >
          ...
          {/* Pricing Box and CTA Alignment Section - Full width on row 3 of mobile grid */}
          <div className="col-start-1 col-end-3 row-start-3 w-full flex flex-row items-center gap-2 mt-1 pt-2 border-t border-[#c4a34f]/10">
            ...
            {/* CTA controls and scarcity links */}
            <div className="flex-1 flex flex-col justify-center gap-1 min-w-0">
              ...
              >
                {/* Backlight pulsing golden aura */}
                <div className="btn-golden-backlight" />
                ...
                <motion.a
                    ...
                    if (typeof window !== "undefined") {
                      ...
                      try {
                        window.gtag?.("event", "begin_checkout", {
                          ...
                          currency: "BRL",
                          items: [{
                            item_id: "cafe_sem_migalhas",
                            item_name: "Café sem Migalhas",
                            price: Number(PRICE_CASH),
                            quantity: 1
                          }]
                        });
                      } catch (err) {
                  ...
                  className="btn-glitter-gold text-center relative z-10 w-full no-underline"
                >
          ...
          {/* Footer alternative rejection link - Full width on row 4 of mobile grid */}
